...
methods: {
    ...
    const t = [];
    const x =
        JSON.parse(localStorage.getItem(`form-table-${this.nome}`)) || f;
    x.forEach((i) => {
    ...
    document.querySelector('table').onfocus = (e) => {
        this.disableScroll();

        document.activeElement == e.target ? this.tableFocused = true : this.tableFocused = false;
        this.$set(this.selectedCell, 'initNavigation', true);
    }

    document.querySelector('table').onblur = (e) => {
        this.enableScroll();

        document.activeElement != e.target ? this.tableFocused = false : this.tableFocused = true
        this.$set(this.selectedCell, 'initNavigation', false)
    }

    document.onkeydown = this.verification;
    },
    verification(e) {
    if (!this.tableFocused && this.paginate.pages > 1) {
        // apenas a navegação de páginas
        this.controlPagesNavigation(e);

    } else if (this.tableFocused && this.paginate.currentPageContent.length != 0) {
        // apenas célular e linhas
        this.controlCellsNavigation(e);
        this.controlRowsNavigation(e);
    ...
    const currentSelectedRow = document.querySelector('.selected');
    const columnsSelected = this.columnsSelected;
    const sliderEl = this.$refs.tableComponent;
    const selectedCell = this.selectedCell;
    const selectColumn = this.selectColumn;
    ...
    const lineHeight = document.querySelector("tbody").firstElementChild.offsetHeight;
    ...
    setTimeout(() => {
        ...
        const resizeEl = document.querySelectorAll('.table-component th .resize-column');
